...
public class InitializeNBody_Task extends Task {
    ...
    public InitializeNBody_Task() {
        ...
        ins[0] = new InputPort("positions", Port.DATAVIEW_MathMatrix,
                "The initial positions of the N-bodies");
        ins[1] = new InputPort("velocities", Port.DATAVIEW_MathMatrix,
                "The initial velocities of the N-bodies");
        ins[2] = new InputPort("masses", Port.DATAVIEW_MathVector,
                "The masses of the N-bodies (CONSTANT)");
        // output ports
    ...
    public void run() {
        ...
        for (int body = 0; body < NBodyWorkflow.N; body++) {
            ...
            Vector3D position = new Vector3D(initialPositions.get(body, 0),
                                             initialPositions.get(body, 1), initialPositions.get(body, 2));
            Vector3D velocity = new Vector3D(initialVelocities.get(body, 0),
                                             initialVelocities.get(body, 1), initialVelocities.get(body, 2));
