...
public class NBodyWorkflow extends Workflow {
	...
	public void design() {
		...
		// workflow inputs
		addEdge(0, initialize, 0); // position input file //Added inputPortIndex 0
		...
		for (int taskInstance = 0; taskInstance < N; taskInstance++) {
			...
			for (int body = 0; body < N; body++) {
				if (body == taskInstance) {
					addEdge(initialize, 3 * body, computeAccelerations[taskInstance], 0);

				} else {
					addEdge(initialize, 3 * body, computeAccelerations[taskInstance], currentInputPort++);
					addEdge(initialize, 3 * body + 2, computeAccelerations[taskInstance], currentInputPort++);
				}
